...
struct DBImpl::CompactionState {
  ...
  //5.9 增加分区信息
  std::vector<std::pair<size_t, Output> > outputs;
  ...
  Output* current_output() { return &(outputs[outputs.size()-1].second); }  //5.9 做相应修改
...
DBImpl::DBImpl(const Options& raw_options, const std::string& dbname)
    : env_(raw_options.env),
      ...
      dbname_(dbname),
      data_dir_("/mnt/sdc/HDB-data"),   //5.7 初始化时赋值
      table_cache_(),
      db_lock_(NULL),
      mutex_(),
      shutting_down_(NULL),
  ...
  table_cache_ = new TableCache(dbname_, data_dir_, &options_, table_cache_size);  //5.7
...
//3.21 删除旧的log
void DBImpl::DeleteObsoleteLog(uint64_t number)
{
  ...
  char fn[10];
  snprintf(fn, 10, "%06d", number);
  //std::string filename = fn;
  //std::cout <<  filename << std::endl;
  env_->DeleteFile(dbname_ + "/" + fn + ".log");
}

//3.21 删除旧的sst
void DBImpl::DeleteObsoleteLdb(std::vector<uint64_t> numbers)
{
  ...
  //filename << number << ".log";
    ...
    char fn[10];
    snprintf(fn, 10, "%06d", numbers[i]);
  //std::string filename = fn;
  //std::cout <<  filename << std::endl;
    env_->DeleteFile(dbname_ + "/" + fn + ".sst");   //5.6 2个路径文件删除
    env_->DeleteFile(data_dir_ + "/" + fn + ".sst");
  }
...
void DBImpl::DeleteObsoleteFiles() {
  ...
  //存在一种情况，文件已创建，但从pending_outputs_中删除，但是还没有写到files_中，这些文件会被误删
  env_->GetChildren(dbname_, &metafilenames); // Ignoring errors on purpose
  //5.6 获取数据文件夹中的文件
  env_->GetChildren(data_dir_, &datafilenames);
  ...
  for (size_t i = 0; i < metafilenames.size(); i++) {
    if (ParseFileName(metafilenames[i], &number, &type)) {
      bool keep = true;
      switch (type) {
        case kLogFile:
      ...
      if (!keep) {
        ...
        Log(options_.info_log, "Delete type=%d #%lld\n",
            int(type),
            static_cast<unsigned long long>(number));
        env_->DeleteFile(dbname_ + "/" + metafilenames[i]);
      }
    }
  }
  //5.8 这个目录中只有数据文件
  for (size_t i = 0; i < datafilenames.size(); i++) {
    if (ParseFileName(datafilenames[i], &number, &type)) {
      ...
      if (!keep) {
        ...
        Log(options_.info_log, "Delete type=%d #%lld\n",
            int(type),
            static_cast<unsigned long long>(number));
        env_->DeleteFile(data_dir_ + "/" + datafilenames[i]);
      }
    }
  }
}
...
Status DBImpl::RecoverLogFile(uint64_t log_number,
  ...
  while (reader.ReadRecord(&record, &scratch) &&
    ...
    if (mem->ApproximateMemoryUsage() > options_.write_buffer_size) {
      ...
      status = WriteL0Table(mem, edit, NULL);
      if (!status.ok()) {
...
//1.26
Status DBImpl::WriteL0Table(MemTable* mem, VersionEdit* edit, Version* base){
  mutex_.AssertHeld();
  ...
  for (size_t i = 0; i < par_filemeta.size(); i++){
    ...
    par = par_filemeta[i].first;   
    // Note that if file_size is zero, the file has been deleted and
    // should not be added to the manifest.   
    if (me.file_size > 0) {
      //给文件选择层次
      /*
      const Slice min_user_key = me.smallest.user_key();
      const Slice max_user_key = me.largest.user_key();
      if (base != NULL) {
      ...
      */
      //3.23将文件放在LO对应的分区bucket中
      //std::cout << me.number << '\n';    //4.10
      edit->AddL0File(par, me.number, me.file_size,
                    me.smallest, me.largest);

      CompactionStats stats;
      stats.micros = env_->NowMicros() - start_micros;
      stats.bytes_written = me.file_size;
...
void DBImpl::CompactMemTableThread() {
  ...
  while (!shutting_down_.Acquire_Load()) {
    ...
    //std::vector<FileMetaData> metas;
    Status s = WriteL0Table(imm_, &edit, base);
...
void DBImpl::CompactLevelThread() {
  ...
  while (!shutting_down_.Acquire_Load()) {
    ...
    while (!shutting_down_.Acquire_Load() &&
      ...
      //NeedsCompaction用来判断当前需要不需要进行Level compaction,如果不需要就wait，需要就执行
      bg_compaction_cv_.Wait();
    }
    if (shutting_down_.Acquire_Load()) {
      break;
...
Status DBImpl::BackgroundCompaction() {
  ...
  if (c == NULL) {
    ...
    c->ReleaseInputs();
    //1.31防止memtable也在compaction时删掉正在创建的文件
    /*
    if (!flag_mem){
      DeleteObsoleteFiles();
    }
...
Status DBImpl::OpenCompactionOutputFile(CompactionState* compact, size_t par) {
  assert(compact != NULL);
  ...
  uint64_t file_number;
  {
    ...
    out.smallest.Clear();
    out.largest.Clear();
    compact->outputs.push_back(std::make_pair(par, out));
    mutex_.Unlock();
  }

  // Make the output file
  std::string fname = TableFileName(data_dir_, file_number);   //5.7 Level compaction中全是L1-L6的文件，所以直接改
...
Status DBImpl::InstallCompactionResults(CompactionState* compact) {
  mutex_.AssertHeld();
  size_t l = compact->compaction->level();
  //5.9 不同层不同日志格式
  if (l < config::kNumLevels - 1){
    Log(options_.info_log,  "Compacted %lu@ %d:%d + %lu@%d files => %lld bytes",
      compact->compaction->num_input_files(0),
      compact->compaction->par(),
      compact->compaction->level()
      compact->compaction->num_input_files(1),
      compact->compaction->level() + 1,
      static_cast<long long>(compact->total_bytes));
  }else{
    Log(options_.info_log,  "Compacted %lu@ %d:%d + %lu files => %lld bytes",
      compact->compaction->num_input_files(0),
      compact->compaction->par(),
      compact->compaction->level(),
      compact->compaction->num_input_files(1),
  ...
  int par = compact->compaction->par();   //5.9
  compact->compaction->AddInputDeletions(compact->compaction->edit(), level, par);  //5.10
  
  for (size_t i = 0; i < compact->outputs.size(); i++) {
    const CompactionState::Output& out = compact->outputs[i].second;   //5.10
    int par = compact->outputs[i].first;
...
//5.9 改好
Status DBImpl::CompactL0(CompactionState* compact, Iterator* input){
  input->SeekToFirst();
  Status status;
    ...
    if (!ParseInternalKey(key, &ikey)) {
      ...
      if (!has_current_key ||
          user_comparator()->Compare(ikey.user_key,
                                     current_key.user_key) != 0) { //5.6 没有删除bound
        /*
  ...
  status = FinishCompactionOutputFile(compact, input);   //5.9 将所有数据写入一个文件
  return status;
}

//5.9 改好
Status DBImpl::CompactL1(CompactionState* compact, Iterator* input){
  input->SeekToFirst();
  Status status;
  ParsedInternalKey ikey;
  ParsedInternalKey current_key;
  ...
  for (; input->Valid() && !shutting_down_.Acquire_Load(); ) {
    ...
    if (!drop) {
      ...
      thn = thn.substr(8,2);
      if (compact->builder == NULL) {       
        std::stringstream ss(thn);
        ss>>par;
        status = OpenCompactionOutputFile(compact, par);
        pre_thn = thn;
        if (!status.ok()) {
          break;
        }
...
//5.9 改好
Status DBImpl::CompactL2(CompactionState* compact, Iterator* input){
  input->SeekToFirst();
  Status status;
  ParsedInternalKey ikey;
  ParsedInternalKey current_key;
...
Status DBImpl::DoCompactionWork(CompactionState* compact) {
  ...
  //5.9 不同层不同日志格式
  if (l < config::kNumLevels - 1){
    Log(options_.info_log,  "Compacting %lu@ %d:%d files",
      compact->compaction->num_input_files(0),
      compact->compaction->par(),
      compact->compaction->level());
  }else{
    Log(options_.info_log,  "Compacting %lu@ %d:%d + %lu files",
      compact->compaction->num_input_files(0),
      compact->compaction->par(),
      compact->compaction->level(),
      compact->compaction->num_input_files(1));
  ...
  mutex_.Unlock();
  //5.9 区分不同Level compaction情况
  //L0将某分区合并到P1层的相同分区的一个文件；L1将某分区的文件合并到P2层的最多10个分区；
  //L2将某分区bucket与同分区overlap文件合并，分区内文件size固定
  Iterator* input = versions_->MakeInputIterator(compact->compaction);
  Status status;
  if (compact->compaction->level() == 0){
    status = CompactL0(compact, input);
  }else if (compact->compaction->level() == 1){   
    status = CompactL1(compact, input);   //5.9 不同前一个函数的地方在于需要分析key
  }else{
    status = CompactL2(compact, input);   //5.9 不同前一个函数的地方在于需要切分文件，大小固定
  }
  ...
  for (size_t i = 0; i < compact->outputs.size(); i++) {
    stats.bytes_written += compact->outputs[i].second.file_size;   //5.9
  }

  mutex_.Lock();
  //5.9 由于第三层存在自compaction，因此需要改动
  if (compact->compaction->level() < config::kNumLevels-1){
...

  {
    ...
    while (true) {
      if (!bg_error_.ok()) {
        ...
        //4.3 记录将compact的imm的log number
        cm_logno = logfile_number_;
...
Status DestroyDB(const std::string& dbname, const Options& options) {
  Env* env = options.env;
  std::vector<std::string> metafilenames;
  std::vector<std::string> datafilenames;   //5.7
  // Ignore error in case directory does not exist
  env->GetChildren(dbname, &metafilenames);
  //5.7 获取另一个盘中的文件
  env->GetChildren("/mnt/sdc/HDB-data", &datafilenames);
  if (metafilenames.empty() && datafilenames.empty()) {   //5.7
    return Status::OK();
  }
  ...
  if (result.ok()) {
    ...
    for (size_t i = 0; i < metafilenames.size(); i++) {
      if (ParseFileName(metafilenames[i], &number, &type) &&
        ...
        Status del = env->DeleteFile(dbname + "/" + metafilenames[i]);
        if (result.ok() && !del.ok()) {
          result = del;
        }
      }
    ...
    //5.7 删除数据文件
    char buf[100];
    snprintf(buf, sizeof(buf), "/mnt/sdc/HDB-data");
    std::string ddir = buf;
    for (size_t i = 0; i < datafilenames.size(); i++) {
      if (ParseFileName(datafilenames[i], &number, &type) &&
          type != kDBLockFile) {  // Lock file will be deleted at end
        Status del = env->DeleteFile(ddir + "/" + datafilenames[i]);
        if (result.ok() && !del.ok()) {
          result = del;
        }
      }
