...
class DBImpl : public DB {
 ...
 private:
  ...
  //3.21
  void DeleteObsoleteLog(uint64_t number);

  //3.21
  void DeleteObsoleteLdb(std::vector<uint64_t> numbers);

  //4.3 记录Level compaction删除的文件号
  std::vector<uint64_t> cl_ldbs;
  ...
  //1.26
  Status WriteL0Table(MemTable* mem, VersionEdit* edit, Version* base) 
    EXCLUSIVE_LOCKS_REQUIRED(mutex_);
  ...
  Status DoCompactionWork(CompactionState* compact)
      EXCLUSIVE_LOCKS_REQUIRED(mutex_);
  Status OpenCompactionOutputFile(CompactionState* compact, size_t par);  //5.9 主要为L1compaction时分区用
  Status FinishCompactionOutputFile(CompactionState* compact, Iterator* input);
  Status InstallCompactionResults(CompactionState* compact)
      EXCLUSIVE_LOCKS_REQUIRED(mutex_);

  //5.9 将三个compact函数拆分
  Status CompactL0(CompactionState* compact, Iterator* input);
  Status CompactL1(CompactionState* compact, Iterator* input);
  Status CompactL2(CompactionState* compact, Iterator* input);
  ...
  bool owns_info_log_;
  bool owns_cache_;
  const std::string dbname_;   //5.6存放元数据信息、LOG和L0层文件

  //5.6 将L1-L6文件存放在另一个磁盘
  ...
  //4.3 记录正在compact的imm的log number
  uint64_t cm_logno;
  ...
  //1.31
  bool flag_mem;  //用于控制compactionLevel时，如果memtable也在compaction，则不deleteobsoletefiles

  std::vector< std::pair<int, FileMetaData> > par_filemeta;  //3.23临时存放memtable compaction时，文件号与分区之间的关系
