...
class Version {
 ...
 private:
  ...
  //3.15数据大小是内存分割时的粒度，如果以1位分割，则长度为10【细粒度会导致小文件】
  std::vector<FileMetaData*> l0files[config::kNumPars];

  //5.8 增加第一层分区bucket
  std::vector<FileMetaData*> p1buckets[config::kNumPars];

  //5.8 增加第二层分区bucket和文件
  std::vector<FileMetaData*> p2buckets[config::kNum2Pars];
  std::vector<FileMetaData*> p2files[config::kNum2Pars];
  ...
  //5.8 增加P1\p2层索引
  std::set<std::pair<int,int>, ParComp> p1_compact_indexes;
  std::set<std::pair<int,int>, ParComp> p2_compact_indexes;

  explicit Version(VersionSet* vset)
      : vset_(vset), next_(this), prev_(this), refs_(0),
        file_to_compact_(NULL), l0_compact_indexes(), l0file_number(0),  //3.29初始化l0_compact_indexes
...
class VersionSet {
 ...
 private:
  ...
  void SetupOtherInputs(Compaction* c, size_t par);
...
class Compaction {
 public:
  ...
  //5.9 增加分区号，第3层也是自分区bucket中的文件和分区内文件compaction
  unsigned par() const {return par_;}
  ...
  void AddInputDeletions(VersionEdit* edit, int level, int par);   //5.9 删除时需要指定分区
