...
static Iterator* GetFileIterator(void* arg,
  ...
  if (file_value.size() != 16) {
    ...
    return cache->NewIterator(options,
                              ...
                              DecodeFixed64(file_value.data() + 8), 1);  //5.6 要么是L0层的要么是非0层的
  }
}
...
void Version::AddSomeIterators(const ReadOptions& options, uint64_t num,
  ...
  */
  for (size_t i = 0; i < config::kNumPars; i++) {
    for(size_t j = 0; j < l0files[i].size(); j++){
      iters->push_back(
        vset_->table_cache_->NewIterator(
            options, l0files[i][j]->number, l0files[i][j]->file_size, 0));
    }
...
Status Version::Get(const ReadOptions& options,
  ...
  for (unsigned level = 0; level < config::kNumLevels; level++) {   //4.4
    
    if (level == 0) {
      // Level-0 files may overlap each other.  Find all files that
      // overlap user_key and process them in order from newest to oldest.
      //4.4 从L0中查找数据，提出key的那一位，然后直接去那个分区找
      std::string uk = user_key.data();
      int par;
      uk = uk.substr(8,1);    //4.4 需要再确定一下，与分区位对应********************
      std::stringstream ss(uk);
      ss>>par;
      num_files = l0files[par].size();
      if (num_files == 0) continue;
    ...
    for (uint32_t i = 0; i < num_files; ++i) {
      ...
      saver.value = value;
      s = vset_->table_cache_->Get(options, level, f->number, f->file_size,
...
void Version::GetOverlappingInputs(
  ...
  for (size_t i = 0; i < p2files[par].size(); ) {
    FileMetaData* f = p2files[par][i++];
...
class VersionSet::Builder {
 private:
  ...
  //3.27 存放每次CompactMemetable后，L0层新增的文件
  std::vector< std::pair<int, FileMetaData> > l0;

  //5.8 存放第一层的新文件
  std::vector< std::pair<int, FileMetaData> > p1b;

  //5.8 存放第二层的新文件
  std::vector< std::pair<int, FileMetaData> > p2b;
...
#pragma GCC diagnostic pop
  ...
  void Apply(VersionEdit* edit) {
    ...
    for (size_t i = 0; i < edit->compact_pointers_.size(); i++) {
      const unsigned level = edit->s_[i].first;
    ...
    for (size_t i = 0; i < edit->new_files_.size(); i++) {
      ...
      levels_[level].deleted_files.erase(f->number);    //有些文件只是移动而不删除
      levels_[level].added_files->insert(f);
    }

    //3.27 Add new L0 files
    for (size_t i = 0; i < edit->new_l0_files_.size(); i++) {
      FileMetaData* f = new FileMetaData(edit->new_l0_files_[i].second);
      f->refs = 1;
      f->allowed_seeks = (f->file_size / 16384);
      if (f->allowed_seeks < 100) f->allowed_seeks = 100;
      l0.push_back(std::make_pair(edit->new_l0_files_[i].first, *f)); 
    }
    //5.8 add new p1 files
    for (size_t i = 0; i < edit->new_p1_files_.size(); i++) {
      FileMetaData* f = new FileMetaData(edit->new_p1_files_[i].second);
      f->refs = 1;
      f->allowed_seeks = (f->file_size / 16384);
      if (f->allowed_seeks < 100) f->allowed_seeks = 100;
      p1b.push_back(std::make_pair(edit->new_p1_files_[i].first, *f)); 
    }

    //5.8 add new p2 bucket files
    for (size_t i = 0; i < edit->new_p2_files_.size(); i++) {
      FileMetaData* f = new FileMetaData(edit->new_p2_files_[i].second);
      f->refs = 1;
      f->allowed_seeks = (f->file_size / 16384);
      if (f->allowed_seeks < 100) f->allowed_seeks = 100;
      p2b.push_back(std::make_pair(edit->new_p2_files_[i].first, *f)); 
    }
  ...
  void SaveTo(Version* v) {
    BySmallestKey cmp;
    cmp.internal_comparator = &vset_->icmp_;
    //3.27 L0的文件增加和删除，compactMemetable时是增加L0文件，L0compaction时是删除L0文件
    int k = 0;
    for(size_t i = 0; i < config::kNumPars; i++){
      const std::vector<FileMetaData*>& base_files = base_->l0files[i];
      for (int j = 0; j < base_files.size(); j++){
        FileMetaData* fm = base_files[j];
        if (levels_[0].deleted_files.count(fm->number) > 0){

        }else{
          v->l0files[i].push_back(fm);
          v->l0file_number++;
          //std::cout << "In SaveTo:" << fm->number << '\n';
        }

      }
      /*
      std::vector<FileMetaData*>::const_iterator it;
      for (it = base_files.begin(); it != base_files.end(); it++){
        if (levels_[0].deleted_files.count((*it)->number) > 0){
          //删除的文件
        }else{
          v->l0files[i].push_back(*it);
          std::cout << "In SaveTo:" << (*it)->number;
        }
      }
      */
      //v->l0files[i].assign(base_->l0files[i].begin(), base_->l0files[i].end());

      while (k < l0.size()){  //同时遍历l0files和l0因为l0中的文件是按分区有序的
        int par = l0[k].first;
        if (par >= i){
          if (par == i){
            FileMetaData* f = new FileMetaData(l0[k].second);
            v->l0files[par].push_back(f);
            v->l0file_number++;
            //std::cout << "In SaveTo:" << f->number << '\n';
          }          
          break;
        }else{
          k++;
    ...
    for (unsigned level = 1; level < config::kNumLevels; level++) {
      ...
      for (FileSet::const_iterator added_iter = added->begin();
           ...
           ++added_iter) {
        // Add all smaller files listed in base_排序
  ...
  void MaybeAddFile(Version* v, unsigned level, FileMetaData* f) {
    if (levels_[level].deleted_files.count(f->number) > 0) {
      ...
      f->refs++;
      files->push_back(f);   //3.28 files_赋值
    }
  }
};
...
//5.4 docompaction时写文件时会释放锁，会不会导致memtable compaction时，finalize选中相同的L0分区？
void VersionSet::Finalize(Version* v) {
  // Compute the ratio of disk usage to its limit
  for (int i = 0; i < config::kNumPars; i++){
        int num = v->l0files[i].size();
        if (num > 5){   //4.7 当L0分区中文件数量大于5时触发该分区compaction  '&& num > max'
          v->l0_compact_indexes.insert(std::make_pair(num,i));  //5.4 按分区数目排序，再按分区号排序
          //max = num;
          //temp_par = i;        
          //break;  //每次只放入一个L0的分区
        }
  ...
  for (int i = 0; i < config::kNumPars; i++){
        int num = v->p1buckets[i].size();
        if (num > 4){   //4.7 当L0分区中文件数量大于5时触发该分区compaction  '&& num > max'
          v->p1_compact_indexes.insert(std::make_pair(num,i));  //5.4 按分区数目排序，再按分区号排序
          //max = num;
          //temp_par = i;        
          //break;  //每次只放入一个L0的分区
        }
  ...
  for (int i = 0; i < config::kNum2Pars; i++){
        int num = v->p2buckets[i].size();
        if (num > 3){   //4.7 当L0分区中文件数量大于5时触发该分区compaction  '&& num > max'
          v->p2_compact_indexes.insert(std::make_pair(num,i));  //5.4 按分区数目排序，再按分区号排序
          //max = num;
          //temp_par = i;        
          //break;  //每次只放入一个L0的分区
        }
...
const char* VersionSet::LevelSummary(LevelSummaryStorage* scratch) const {
  ...
  snprintf(scratch->buffer, sizeof(scratch->buffer),
           "files[ %d %d %d %d ]",
           int(current_->l0file_number),   //5.3 输出状态获取L0层文件数量
           int(current_->p1b_number),
           int(current_->p2b_number),
           int(current_->p2file_number));
    ...
    for (size_t i = 0; i < files.size(); i++) {
      if (icmp_.Compare(files[i]->largest, ikey) <= 0) {
        ...
        Iterator* iter = table_cache_->NewIterator(
            ReadOptions(), files[i]->number, files[i]->file_size, level, &tableptr);   //5.7
        if (tableptr != NULL) {
...
void VersionSet::AddLiveFiles(std::set<uint64_t>* live) {
  for (Version* v = dummy_versions_.next_;
    ...
    //5.8 修改分区文件号的获取
    for (unsigned i = 0; i < config::kNumPars; i++){
      const std::vector<FileMetaData*>& files = v->p1buckets[i];
      for(size_t j = 0; j < files.size(); j++){
        live->insert(files[j]->number);
      }
    }
    for (unsigned i = 0; i < config::kNum2Pars; i++){
      const std::vector<FileMetaData*>& files = v->p2buckets[i];
      for(size_t j = 0; j < files.size(); j++){
        live->insert(files[j]->number);
      }
      for(size_t j = 0; j < v->p2files[i].size(); j++){
        live->insert(v->p2files[i][j]->number);
      }
    }
    /*
    for (unsigned level = 0; level < config::kNumLevels; level++) {
...
Iterator* VersionSet::MakeInputIterator(Compaction* c) {
  ...
  //5.9 暂时未考虑各bucket中有文件不overlap的情况
  const int space = (c->level() == 0 ? c->inputs_[0].size() + 1 : 2);
  Iterator** list = new Iterator*[space];
  int num = 0;
  for (int which = 0; which < 2; which++) {
    if (!c->inputs_[which].empty()) {
      if (c->level() + which < config::kNumLevels+1) {  //5.9 非最后一层的自compaction文件
        const std::vector<FileMetaData*>& files = c->inputs_[which];
        for (size_t i = 0; i < files.size(); i++) {
          list[num++] = table_cache_->NewIterator(
              options, files[i]->number, files[i]->file_size, 0);  //给函数增加一个参数，为level
        }
      } else {
...
unsigned VersionSet::PickCompactionLevel(bool* locked, bool seek_driven) const {
  ...
  bool found = false;
  if (!current_->l0_compact_indexes.empty()){
      std::set<std::pair<int,int> >::iterator it;
      it = current_->l0_compact_indexes.begin();
      //4.24 判断当前第一个compact index是否重复的     
      while (it != current_->l0_compact_indexes.end()){
        int par = it->second;
        if (l0files[par].size() > 5){
          found = true;
          level = 0;
          break;
        }
  ...
  if (!found){
    if (!current_->p1_compact_indexes.empty()){
      std::set<std::pair<int,int> >::iterator it;
      it = current_->p1_compact_indexes.begin();
      //4.24 判断当前第一个compact index是否重复的     
      while (it != current_->p1_compact_indexes.end()){
        int par = it->second;
        if (l0files[par].size() > 4){
          found = true;
          level = 1;
          break;
        }
  ...
  if (!found){
    if (!current_->p2_compact_indexes.empty()){
      std::set<std::pair<int,int> >::iterator it;
      it = current_->p2_compact_indexes.begin();
      //4.24 判断当前第一个compact index是否重复的     
      while (it != current_->p2_compact_indexes.end()){
        int par = it->second;
        if (l0files[par].size() > 3){
          found = true;
          level = 2;
          break;
        }
...
Compaction* VersionSet::PickCompaction(Version* v, unsigned level) {
  ...
  */
  ...
  if (level < config::kNumLevels-1){
    if (level == 0){
      if (!v->l0_compact_indexes.empty()){
        //write compaction
        std::set<std::pair<int,int> >::iterator it;
        it = v->l0_compact_indexes.begin();
        par = it->second;       
        //std::cout << "picked par:" << par << '\n';    //4.7  
        if (v->l0files[par].empty()){
          return NULL;
        }     
        tmp(v->l0files[par]);        
        ...
        //seek compaction
        par = SeekCompaction(v->l0files, 10, tmp);
      }
      ...
      //level 1
      if (!v->p1_compact_indexes.empty()){
        //write compaction
        std::set<std::pair<int,int> >::iterator it;
        it = v->p1_compact_indexes.begin();
        par = it->second;       
        //std::cout << "picked par:" << par << '\n';    //4.7
        if (v->p1buckets[par].empty()){
          return NULL;
        }
        tmp(v->p1buckets[par]);
        std::sort(tmp.begin(), tmp.end(), OldestFirst);
        ...
        //seek compaction
        par = SeekCompaction(v->p1buckets, 10, tmp);
      }
      for (size_t i = 0; i < tmp.size() && c->inputs_[0].size() < 32; ++i) {
        c->inputs_[0].push_back(tmp[i]);
      }
    ...
    //level 2, compact the data in buckets to files in the partition
    if (!v->p2_compact_indexes.empty()){
      //write compaction
      std::set<std::pair<int,int> >::iterator it;
      it = v->p2_compact_indexes.begin();
      par = it->second;       
      //std::cout << "picked par:" << par << '\n';    //4.7
      if (v->p2buckets[par].empty()){
        return NULL;
      }
      tmp(v->p2buckets[par]);
      std::sort(tmp.begin(), tmp.end(), OldestFirst);
      ...
      //seek compaction
      par = SeekCompaction(v->p2buckets, 100, tmp);
    }
    for (size_t i = 0; i < tmp.size() && c->inputs_[0].size() < 32; ++i) {
      c->inputs_[0].push_back(tmp[i]);
    }
    SetupOtherInputs(c, par);
  }
  c->par_ = par;   //5.9 增加分区信息 
  //5.8 暂时没考虑trivial的情况，一般发生在sequential load的时候，其实在分区内部进行判断是否overlap即可，先把整体改好，编译通过吧 
/*
  ...
  if (!trivial) {
    //std::cout << "start SetupOtherInputs" << '\n';
    SetupOtherInputs(c);
    //std::cout << "finish SetupOtherInputs" << '\n';
  }
  */
  return c;
}
...
//5.8 seek compaction
size_t VersionSet::SeekCompaction(std::vector<FileMetaData*> files[], size_t pnum, std::vector<FileMetaData*> &tmp) {
  size_t i = 0;
  for (; i < pnum; i++){
        int num = files[i].size();
        if (num > 0){
          std::vector<FileMetaData*> tmp(files[i]);
          if (num > 1){
            std::sort(tmp.begin(), tmp.end(), OldestFirst);
          }
  ...
  assert(i < config::kNumPars);   //当选中L0层COM的时候，必定能选到文件，如果L0已空，报错
  return i;   //5.8 返回分区号
}
//5.8
void VersionSet::SetupOtherInputs(Compaction* c, size_t par) {
  const unsigned level = c->level();
  assert(level < config::kNumLevels);
  InternalKey smallest, largest;
  GetRange(c->inputs_[0], &smallest, &largest);
  c->input_version_->GetOverlappingInputs(par, &smallest, &largest, &c->inputs_[1]);
  /*
...
void Compaction::AddInputDeletions(VersionEdit* ed, int level, int par) {
  ed->level = level;
  for (int which = 0; which < 2; which++) {
    for (size_t i = 0; i < inputs_[which].size(); i++) {
      if (level+which < config::kNumLevels){
        ed->DeleteFile(par, inputs_[which][i]->number);  //5.9
      } else{
        ed->DeleteP2File(par, inputs_[which][i]->number);  //5.10  将第二层要删除的有序文件单独列出
      }     
